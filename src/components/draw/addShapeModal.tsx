...
import {addMap} from '../../context/slices/mapSlice'
import {serializeLatlngs} from '../../helpers/serializeLatLangs'
import {GetLatLangs} from '../../helpers/detectLayer'
...
function AddShapeModal(props) {
  ...
  const submitForm = (event) => {
    ...
    if (!data) return
    const latlngs = GetLatLangs(data.layer)
    const serializedLatlngs = serializeLatlngs(latlngs)

    const object = {
      id: data.layer._leaflet_id,
      name,
      layerType: data.layerType,
      latlngs: serializedLatlngs,
    }

    dispatch(addMap(object))
    setShow(false)
    setShowToast(true)
    setTimeout(() => {
      setShowToast(false)
